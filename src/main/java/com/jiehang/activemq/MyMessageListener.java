...
public class MyMessageListener implements MessageListener {
    ...
    ExecutorService executor = new ThreadPoolExecutor(8, 10, 3, TimeUnit.MINUTES, new ArrayBlockingQueue<>(0));
    @Override
    public void onMessage(Message message) {
        if (((ThreadPoolExecutor) executor).getActiveCount() < ((ThreadPoolExecutor) executor).getMaximumPoolSize()) {
